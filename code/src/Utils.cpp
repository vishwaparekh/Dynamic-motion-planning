...
bool SegmentCircleIntersection(const double x1, const double y1, const double x2, const double y2,
    const double cx, const double cy, const double r){
        //temporary variables to maintain current position on the segment.
        double x = x1, y = y1;
        //gradient angle
        double theta = atan2((y2-y1),(x2-x1));
        //step size, the lenght of small edges we want to break our segment into
        double eps = 0.05;
        //until we have not iterated through the segment, go to every point/ end points of these small segments and check if these points lie inside the given circle or not
        double length = EuclideanDistance(x2, y2, x1, y1);
        while(EuclideanDistance(x, y, x1, y1) < length){
            if (!IsPointInsideCircle(x, y, cx, cy, r)){
                x += eps * length * cos(theta);
                y += eps * length * sin(theta);
            }
            else{
                return true;
            }
...
{
    //for each of the segment on the polygon, compute SegmentCircleIntersection
    for (int i = 0; i < n; i++){
        int i1 = (i + n - 1) % n;
        if (SegmentCircleIntersection(poly[2*i1], poly[2*i1 + 1], poly[2*i], poly[2*i + 1], cx, cy, r)){
            //cerr << "(" << i1 << "," << i << ")" << endl;
            return true;
        }
    }
...
bool PolygonPolygonCollision(const int n, const double poly[], const int n2, const double poly2[])
{
